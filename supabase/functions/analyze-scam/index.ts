...
function analyzeText(input: string): ScamResult {
  ...
  for (const signal of SIGNALS) {
    ...
    if (m) {
      score += signal.weight;
      if (signal.weight > 0) matched.push({ ...signal, matchedText: m[0] });
    }
  }
  ...
  return buildResult(level, signals, matched);
}
...
function buildResult(
  ...
  if (level === "high") {
    ...
    if (hasSignal(matched, [
      ...
      advice.push("This message claims a family member is in trouble. Do not send money. Call that person directly on a phone number you already know to check if they are really okay.");
    }
    if (hasSignal(matched, ["Asks for gift cards as payment", "Asks for a specific brand of gift card", "Wants the gift card codes"])) {
      advice.push("No real business or government office will ever ask you to pay with gift cards. Anyone who does is a scammer. Do not buy or share any gift card codes.");
    }
    if (hasSignal(matched, ["Asks for hard-to-trace payment", "Asks you to send or wire money"])) {
      advice.push("Do not send money through wire transfer, Bitcoin, Cash App, Zelle, or any other hard-to-trace method. Once it is sent, you cannot get it back.");
    }
    if (hasSignal(matched, ["Threatens legal action or arrest", "Claims your Social Security number is suspended"])) {
      advice.push("The real IRS, police, and Social Security office never call to threaten you with arrest over the phone. Hang up — this is a scare tactic.");
    }
    if (hasSignal(matched, ["Wants remote access to your device"])) {
      advice.push("Never let a stranger install software or take remote control of your computer. Hang up and call a trusted family member or a known tech shop.");
    }
    if (hasSignal(matched, ["Asks for passwords or private codes", "Pushes you to log in to a fake page"])) {
      advice.push("Do not give out your password, PIN, or one-time code. A real company will never ask for these over a text, email, or phone call.");
    }
  ...
  if (level === "suspicious") {
    ...
    if (hasSignal(matched, ["Threatens to close your account", "Claims a familiar company is canceling your order or account"])) {
      advice.push("This message claims a problem with your account. Do not click the link. If you are worried, open your app or call the company using the phone number on your card or bill.");
    }
    if (hasSignal(matched, ["Pushes you to click a link", "Contains a web link", "Uses a shortened link that hides the real address"])) {
    ...
    return {
      ...
      summary: "This message has some warning signs. It may not be safe. Take a moment to check with someone you trust.",
      advice,
      signals,
    };
  }
  ...
  return {
    level,
    title: "Likely Safe",
    summary: "We did not find strong scam warning signs in this message. It looks like a normal, friendly message.",
    advice: [
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body || typeof body.text !== "string" || !body.text.trim()) {
      return new Response(JSON.stringify({ error: "Please provide some text to check." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ error: "Something went wrong while checking your message." }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
